...
((Resource2) => {
  ...
  function encode(type, data) {
    ...
    const element = (0, import_element.default)(type, (0, import_cosmokit.pick)(data, resource.attrs));
    for (const key of resource.children) {
      if ((0, import_cosmokit.isNullable)(data[key])) continue;
      element.children.push(encode(key, data[key]));
    }
    if (resource.content && !(0, import_cosmokit.isNullable)(data[resource.content])) {
      element.children.push(...import_element.default.parse(data[resource.content]));
    }
    return element;
  }
